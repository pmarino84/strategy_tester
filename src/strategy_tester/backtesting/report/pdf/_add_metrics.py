...
def _add_metrics(pdf: PdfPages, metrics: Dict[str, Union[pd.DataFrame, pd.Series]]):
  ...
  if not profits_losses_mean_by_hour.empty and not profits_losses_mean_by_dow.empty and not profits_losses_mean_by_month.empty:
    fig, axes = plt.subplots(3, 1, figsize=(16, 14))

    profits_losses_mean_by_hour_colors = profits_losses_mean_by_hour.map(lambda x: "green" if x > 0 else "red").to_list()
    profits_losses_mean_by_hour.plot.bar(ax=axes[0], color=profits_losses_mean_by_hour_colors)
    axes[0].set_title("Profits/Losses Mean by Hour")
    axes[0].legend().remove()

    profits_losses_mean_by_dow_colors = profits_losses_mean_by_dow.map(lambda x: "green" if x > 0 else "red").to_list()
    profits_losses_mean_by_dow.plot.bar(ax=axes[1], color=profits_losses_mean_by_dow_colors)
    axes[1].set_title("Profits/Losses Mean by Day Of Week")
    axes[1].legend().remove()

    profits_losses_mean_by_month_colors = profits_losses_mean_by_month.map(lambda x: "green" if x > 0 else "red").to_list()
    profits_losses_mean_by_month.plot.bar(ax=axes[2], color=profits_losses_mean_by_month_colors)
    axes[2].set_title("Profits/Losses Mean by Month")
    axes[2].legend().remove()
